...
> 答：
（1）数据完整性
含义：接收方能够确认数据在传输过程中没有被修改、丢失、插入字节。
实验实现：发送方对密文encrypted_message.bin做 SHA256 哈希再签名；接收方用发送方公钥验签，重新计算密文哈希对比。篡改密文tampered_encrypted_message.bin后验签直接返回Verification failure，证明数据改动会导致签名校验失败，保证完整性。
（2）身份认证（发送者身份可信）
含义：接收方可确认消息确实由持有对应私钥的发送方发出，中间人无法伪造合法签名。
实验实现：签名仅能用发送方私钥signature_private_key.pem生成；验证只能使用配套公钥signature_public_key.pem。无私钥的攻击者无法生成能通过校验的签名，以此确认发送者身份。
（3）不可否认（抗抵赖）
含义：发送方无法否认自己发送过该消息，私钥唯一归属发送方，签名行为可作为凭证。
实验实现：签名由发送方独有私钥生成，公钥全网公开可验证。一旦签名校验通过，发送方无法否认自己签署过这份密文，具备举证效力。
...
> 答：
（1）RSA 明文长度存在上限限制：
RSA 加密 / 签名的输入数据长度不能超过密钥模数长度（2048 位 RSA 最多输入 245 字节左右）。若密文长度超过该阈值，无法直接签名；SHA256 固定输出 32 字节哈希值，长度永远满足 RSA 输入限制，适配任意大小文件。
（2）大幅提升签名运算效率：
原始密文encrypted_message.bin为 256 字节，若文件是 GB 级大文件，直接签名会带来海量模幂运算；哈希将任意长度数据压缩为固定 32 字节短摘要，RSA 仅对短摘要运算，计算速度提升数个量级。
（3）规避 RSA 签名安全缺陷，统一摘要长度：
若直接对原始数据签名，存在消息碰撞、短消息攻击风险；密码学哈希算法（SHA256）具备抗碰撞、不可逆特性，保证微小数据改动会产生完全不同的摘要，篡改必然验签失败，强化完整性校验能力。
...
> 答：
1、（1）RSA 签名输出等于模数N的字节长度；
（2）2048 bit = 2048 ÷ 8 = 256 字节；
（3）本次实验模数 N 为 2048 位，因此生成的二进制签名文件signature.bin固定 256 字节，与文件内容无关。
2、4096 bit ÷ 8 = 512 字节，使用 4096 位 RSA 私钥签名时，输出签名文件固定为 512 字节。
...
给定哈希输出值 
h
，无法在计算可行的时间内反向推导出原始输入消息 
m
，哈希运算不可逆。
不满足的安全问题
攻击者拿到签名对应的哈希摘要，可逆向还原原始明文，直接窃取完整消息；同时能伪造任意消息的哈希值，绕过签名校验，破坏数据保密性与完整性。
（2）抗第二原像性（弱抗碰撞）
含义
给定一条已知消息 
m1 ，找不到另一条不同消息m2，使得 
hash(m1)=hash(m2)。
不满足的安全问题
攻击者拿到合法消息与签名后，可构造一条恶意替换消息，二者哈希完全一致；恶意消息能通过签名验证，接收方无法区分真假消息，伪造篡改内容。
（3）抗碰撞性（强抗碰撞）
含义
无法找到两组完全不同的消息 
(m1,m2)，满足hash(m1)=hash(m2)。
不满足的安全问题
攻击者可主动生成一对 “合法外观 + 恶意功能” 的碰撞文件，用私钥对无害文件签名，再将签名套用到恶意文件上，完成签名伪造，彻底摧毁数字签名的身份认证与防篡改能力。
---
...
> 答：
一、SHA-1 不再推荐用于数字签名的原因
抗碰撞性已被攻破：2017 年谷歌公布 SHA-1 碰撞攻击，攻击者可低成本构造两条不同文件产生相同哈希摘要，破坏数字签名核心安全前提；
算法设计缺陷：哈希输出仅 160 比特，摘要空间过小，暴力碰撞计算成本持续下降；
行业标准淘汰：NIST、浏览器、CA 证书机构已全面禁用 SHA-1 签名，强制升级 SHA-256/SHA3。
二、2017 年 SHA-1 碰撞攻击对数字签名的影响
数字签名依赖哈希摘要校验完整性，若两条消息哈希相同，同一份签名对两条消息都有效；攻击者可使用一份合法签名，欺骗接收方信任恶意伪造内容，签名的防篡改、身份认证完全失效。
三、攻击举例
攻击者构造两份 PDF 文件：文件 A（正常转账 1 元）、文件 B（转账 10000 元），利用 SHA-1 碰撞漏洞让二者哈希值完全一致；
发送方用私钥对无害文件 A 签名，生成合法签名 signature.bin；
攻击者将文件替换为恶意文件 B，搭配原签名发给接收方；
接收方验签时，计算文件 B 的 SHA-1 哈希与签名内摘要匹配，判定签名合法，最终执行恶意转账操作。
本次实验使用 SHA-256，不存在该碰撞风险，因此能可靠校验密文是否被篡改。
---
...
> 答：
（1）签名密钥对
签名私钥（signature_private_key.pem）：发送方独有，用于对密文哈希进行数字签名操作，生成 signature.bin，证明消息由本人发出。
签名公钥（signature_public_key.pem）：对外公开，用于验证签名操作，接收方校验签名合法性，确认消息未篡改、发送者身份真实。
（2）加密密钥对
加密公钥（encryption_public_key.pem）：接收方对外公开，发送方使用它对原始明文 message.bin 执行加密，生成密文 encrypted_message.bin。
加密私钥（encryption_private_key.pem）：接收方私有，仅自己持有，用于解密密文，还原原始明文 message.txt。
（3）攻击者获取公钥后的权限限制
攻击者可以做：
1、使用加密公钥加密消息，向目标发送密文；
2、使用签名公钥验证任意签名，校验消息完整性；
3、公开公钥本身无保密价值，任何人都可获取。
攻击者不能做：
1、无法解密用该公钥加密的数据（解密依赖对应私钥）；
2、无法伪造合法数字签名（签名依赖私钥，仅有公钥无法生成有效签名）；
3、无法推导、破解出配套私钥。
...
> 答：
一、本质区别（结合实验流程：明文→加密→密文→哈希→签名→传输→验签→解密）
操作主体与密钥使用相反
加密：用接收方公钥加密，只有接收方私钥可解密；目标是保密性。
签名：用发送方私钥签名，所有人可用发送方公钥验签；目标是完整性、身份认证、不可否认。
保护对象不同
加密：保护明文内容，防止第三方窃听读取数据。
签名：保护数据完整性与发送者身份，防止篡改、伪造发送人。
可逆性差异
加密是双向操作：加密后必须解密才能还原原文。
签名是单向证明：仅用于校验，不需要 “解签名”，验签仅比对哈希摘要。
二、各自解决的安全问题
消息加密：解决保密性问题，保证传输过程中只有指定接收方能够读取明文，中间人无法窃取内容。
数字签名：解决三大安全问题：
完整性：数据被篡改后验签失败；
身份认证：确认消息由私钥持有者发送；
不可否认：发送方无法抵赖自己发送过该消息。
三、不能用加密代替签名的原因
1、无法实现不可否认
若用发送方公钥加密消息，任何人都能拿到公钥解密，无法证明消息是谁发出；只有私钥签名才能唯一绑定发送者身份。
2、无法公开校验
加密后的密文只有持有私钥的人能读取、校验，第三方无法独立验证消息真伪；签名公钥可公开分发，任何人都能验签。
3、安全目标完全割裂
加密只管 “不让别人看”，不管 “内容有没有被改、是谁发的”；仅加密无法抵御中间人篡改数据后重新加密发送的攻击（本实验篡改密文后验签直接失败，仅加密无此防护）。
4、场景需求不匹配
很多场景只需要证明数据真实、不需要保密（公开公告、文件公示），此时只需要签名，不需要加密；加密无法满足公开可验证的需求。
---
...
签名的不可否认性
1. 不可否认性定义
不可否认性（Non-repudiation）：消息发送方完成签名 / 发送行为后，事后无法否认自己发送、签署过该消息；接收方可向第三方举证证明发送者行为。
2. 数字签名可提供不可否认，MAC 无法提供的原因
（1）密钥体系差异
数字签名：非对称密码，签名私钥仅发送方独自持有，公钥公开。只有发送方能生成合法签名，第三方可独立用公钥核验，具备举证效力。
MAC：对称密钥，通信双方共享同一密钥，收发双方都能生成合法 MAC 值，无法区分是谁生成的认证码。
（2）抵赖风险
MAC 场景下，发送方可否认自己发送消息，声称是接收方伪造 MAC；数字签名私钥唯一归属发送人，无法抵赖。
---
...
> 答：
场景 1：软件安装包 / 程序发布签名
（1）作用：开发者用私钥对安装包签名，用户通过官方公钥验签。
（2）解决问题：防止安装包被中间人植入病毒、篡改；确认软件来自官方开发者，杜绝钓鱼恶意程序。
场景 2：电子合同、电子政务文件
（1）作用：签署方使用个人 CA 私钥对合同签名。
（2）解决问题：确认签署人真实身份，合同不可篡改，事后签署方无法否认签署行为，具备法律举证效力。
场景 3：Git 代码提交 / 开源代码签名
（1）作用：开发者用 RSA 私钥对 commit 提交签名，仓库使用公钥校验。
（2）解决问题：防止攻击者伪造他人身份提交恶意代码，追溯代码真实作者，保证代码提交完整性。
---
...
> 答：
1、数字签名本身无法证明签名时间
签名仅绑定文件哈希与签名私钥，不含可信时间信息，持有者可修改本地系统时间后重新签名，无法证明真实签署时刻。
2、验证签名时间的方案：可信时间戳服务 TSA
流程：
① 签名完成后，将签名文件哈希发送给第三方可信时间戳机构 TSA；
② TSA 用自身私钥对哈希 + 标准权威时间生成时间戳签名；
③ 将 TSA 时间戳附加到原始签名文件中。
3、纠纷校验逻辑
一年后发生争议时，接收方同时校验：
（1）文件原始数字签名合法；
（2）TSA 时间戳签名有效，提取 TSA 提供的权威签署时间，证明文件实际签署日期，戳破 “昨天才签名” 的抵赖说辞。
---
...
> 答：
一、本实验选用 2048 位 RSA 密钥的原因
2048 位是安全性与性能平衡的通用标准：
1、安全充足：NIST 等权威机构认定 2048 位 RSA 可安全使用至 2030 年，抵御现有暴力分解、量子前的常规破解手段，满足教学、常规业务长期安全需求；
2、性能适中：相比 4096 位，2048 位加解密、签名验签运算速度更快，资源占用更低，普通 PC / 虚拟机可流畅完成实验操作；
3、行业通用：目前互联网证书、软件签名、常规加密场景主流标准，具备通用性，适配 OpenSSL 等工具默认规范。
二、1024 位 RSA 是否安全
不再安全，禁止商用 / 正式场景使用：
1、大数分解算法持续优化，1024 位模数已存在低成本破解案例；
2、NIST 早在 2015 年就淘汰 1024 位 RSA，无法抵御中长期算力攻击，仅可用于无敏感数据的简易教学演示，不能保护真实隐私、业务数据。
三、4096 位 RSA 是否有必要
分场景权衡：
适用（有必要）场景
高密级涉密、金融核心交易、长期有效期根证书、需要抗远期算力 / 量子风险的场景，4096 位安全冗余更高，可延长安全生命周期。
不推荐（无必要）场景
普通文件加密、软件分发、日常实验、高并发业务：4096 位模幂运算计算量是 2048 位的 4 倍以上，CPU / 内存开销大幅上升，签名、解密延迟显著增加，性能损耗严重，普通场景安全收益远抵不上性能损失。
---
...
> 答：
一、Alice 签文件、Bob 签 Alice 的签名，这种做法有意义
该操作属于多级签名 / 签名链，具备明确业务意义，不是无意义嵌套：
1、第一层 Alice 签名：证明文件原始内容由 Alice 生成，保证文件本身完整性、Alice 身份；
2、第二层 Bob 对 Alice 的签名做签名：证明 Bob 审核、认可了 Alice 这份带签名的文件，绑定 Bob 的责任；
两层签名独立校验，可分别追溯两级签署人的行为。
二、需要多重签名 / 签名链的典型场景
1、多级审批流程（企业 / 政务公文）
起草人 Alice 签署文件，部门主管 Bob 复核并对 Alice 的签名二次签名，代表两级均认可文件；审计时可分别校验两层签名，区分起草人与审批人责任。
2、证书信任链（CA 体系）
根 CA 自签根证书，二级 CA 用根证书签发证书（对下级公钥签名），终端证书由二级 CA 签名，形成完整签名链，浏览器逐级验签建立信任。
3、供应链软件分发
开发团队 Alice 签名程序包，发行商 Bob 校验源码后对开发者签名再签名，终端用户可同时验证代码作者与发行方双重可信，防止中间渠道篡改。
4、多方联合合约、电子招投标
甲乙双方依次对对方签名进行嵌套签名，确认双方均认可同一合同文本，具备分层不可否认举证效力。
---
