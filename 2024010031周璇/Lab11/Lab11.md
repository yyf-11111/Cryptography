...
| :--- | :------- |
| 签名 RSA 密钥长度（位） | |2048
| 签名私钥 `signature_private_key.pem` 文件大小（字节） | | 197121（约 1.7K）
| 签名公钥 `signature_public_key.pem` 文件大小（字节） | | 460
| 签名公钥指数 e 的值 | |65537（0x10001）
| 签名公钥模数 N 的前 16 位十六进制 | |00:a2:16:3d:96:3d:d5:ad
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） | |这是一条重要消息，需要签名保护。发送者：周旋，学号：2024010031
| 密文文件 encrypted_message.bin 大小（字节） | |256
| 密文 SHA-256 哈希值（完整的 64 位十六进制） | |96db2e775811bc5c02515541763a9b55dd0aee43be7c29855e7b5174f6b8e
...
| 接收方应先验证签名还是先解密 | |先验证签名，验证通过再解密；验证失败直接丢弃密文
...
> 答：数字签名具备完整性、身份认证（不可伪造）、不可抵赖三大核心保证。
完整性：接收方可确认消息传输中未被篡改、增删。本实验对密文哈希后签名，修改密文任意 1 字节，哈希完全变化，签名校验直接失败，实现完整性校验。
身份认证（不可伪造）：仅私钥持有者能生成合法签名，他人无法冒充。实验使用专属签名私钥生成签名，仅配套公钥可校验，攻击者无私钥无法伪造有效签名。
不可抵赖：签名者签署消息后无法否认。私钥仅本人保管，公钥公开可核验，只要校验通过，即可唯一确定签署人，不存在抵赖依据。
...
> 答：突破 RSA 长度限制：RSA 仅能加密 / 签名小于模数长度的数据，长文件无法直接签名；哈希将任意长度数据压缩为固定短摘要，适配 RSA 输入限制。
大幅提升运算效率：RSA 大数模幂运算速度极慢，仅对 32 字节 SHA256 摘要签名，相比直接对大文件签名极大节省耗时。
抵御代数伪造攻击：直接对明文 RSA 签名存在乘法构造伪造漏洞；单向哈希破坏明文代数结构，杜绝攻击者构造伪造签名。
标准化统一流程：无论文件大小，哈希输出长度固定，签名、验证流程统一，适配全部文件类型。
...
> 答：2048 位 RSA 签名长度 256 字节原因：RSA 签名输出是模模数 N 的整数，字节长度 = 密钥比特数 ÷8，2048÷8=256，因此签名文件固定 256 字节。
4096 位 RSA 密钥签名长度：4096÷8=512，签名长度为 512 字节。
规律：RSA 签名字节长度 = RSA 密钥比特长度 ÷ 8。
...
> 答：哈希函数必须满足抗原像、抗第二原像、抗碰撞三个性质。
抗原像（单向性）：给定哈希输出 h，无法反向找到原文 m。不满足则攻击者可由哈希还原明文，构造虚假文件绕过签名校验。
抗第二原像（弱抗碰撞）：给定 m1，找不到 m2≠m1 使 Hash (m1)=Hash (m2)。不满足则攻击者可制作恶意文件复用合法签名，篡改内容仍能校验通过。
抗碰撞（强抗碰撞）：无法找到两组不同原文哈希完全相同。不满足则攻击者预先构造碰撞文件，先用正常文件获取签名，替换恶意文件完成完整伪造，破坏签名全部安全保证。
...
> 答：淘汰原因：SHA-1 已被攻破，不满足强抗碰撞安全需求，2017 年谷歌公开构造出两组内容不同但 SHA-1 哈希完全一致的碰撞文件，基于 SHA-1 的数字签名存在伪造漏洞，浏览器、证书标准均全面淘汰 SHA-1。
对数字签名的影响：数字签名依靠哈希唯一绑定原文，存在碰撞时恶意文件可复用正常文件的合法签名，接收方校验无法区分真假，完整性、身份认证全部失效。
攻击举例：攻击者制作一对碰撞文件，A 为正常合规文档，B 为恶意篡改文档，二者 SHA-1 摘要相同。攻击者发送 A 获取合法签名，再替换为 B 发送；接收方计算 B 的哈希与签名内摘要匹配，误判文件合法，伪造攻击成功。
...
> 答：签名密钥对
签名私钥：持有者私密保存，用于对文件哈希摘要生成数字签名。
签名公钥：对外公开分发，所有人可使用它校验数字签名合法性。
加密密钥对
加密公钥：对外公开，其他人使用公钥加密明文，密文仅配套私钥可解密。
加密私钥：持有者私密保存，用于解密他人用该公钥加密后的密文。
攻击者获取公钥后
能做：验证对应数字签名；使用公钥加密消息发给密钥持有者。
不能做：生成合法数字签名；解密公钥加密的密文；推导、还原配套私钥
...
> 答：本质区别：消息加密核心解决保密性，防止第三方读取内容；数字签名核心解决完整性、身份认证、不可抵赖，防止篡改、冒充、抵赖，不隐藏明文。
结合实验流程说明：加密使用接收方公钥隐藏原文实现保密；哈希 + 签名使用发送方私钥绑定原文与发送人身份；验证用发送方公钥确认文件未篡改、发送人真实；解密用接收方私钥读取原始消息。
不能用加密代替签名的原因：
任何人都能获取接收方公钥加密消息，无法确认发送者身份，无身份认证；
加密无唯一可溯源凭证，发送方可以抵赖发送行为；
单纯加密无哈希校验机制，密文篡改后只会输出乱码，无法精准校验完整性；
加密依赖接收方公钥，签名依赖发送方私钥，底层逻辑完全相反，无法互换。
...
> 答：不可否认性（Non-repudiation）：消息签署完成后，签名者无法事后否认自己签署、发送过该消息，第三方仲裁机构可凭签名判定责任归属。
数字签名具备、MAC 不具备的核心原因：密钥体系不同。
数字签名基于非对称密码：签名私钥唯一归属发送人，仅本人持有，公钥全网公开可核验，合法签名只能由私钥持有人生成，责任唯一，具备不可否认能力。
MAC 消息认证码基于对称密码：通信双方共享同一组密钥，收发两方都能生成合法 MAC 校验码，出现恶意消息无法区分是谁伪造，无唯一责任人，无法实现不可否认性。
...
> 答：软件安装包数字签名：操作系统校验安装包签名，确认软件来自正规厂商、传输未被篡改，防止木马篡改安装程序。
代码仓库提交签名（Git GPG 签名）：开发者使用私钥对代码提交签名，证明代码确实由该作者编写，防止他人冒充身份提交恶意代码。
电子合同 / 电子发票签名：企业使用数字证书私钥签署电子票据，具备法律不可否认性，可作为司法凭证，替代纸质盖章。
...
> 答：数字签名本身无法证明签名的生成时间，仅能证明私钥持有人签署过文件，无法区分签署时间。
验证签名时间的解决方案：签署文件后，将文件 + 签名发送至可信时间戳服务（TSA），TSA 使用权威时间源生成带官方时间的时间戳并签名；一年后校验时，同时验证文件签名与 TSA 时间戳，即可确定准确签署时间，防止篡改签署时间。
...
> 答：实验选用 2048 位 RSA 原因：平衡安全性与运算性能，符合当前行业最低安全标准，可抵御中长期已知大数分解攻击。
1024 位 RSA：不再安全，现代算力可在短时间内分解模数，容易被破解，禁止用于正式业务。
4096 位 RSA：安全性更高，抗量子前置攻击能力更强，但模幂运算耗时大幅提升，加密、签名、验证速度显著变慢，仅高安全等级场景（根证书、核心密钥）使用。
权衡逻辑：普通业务优先 2048 位兼顾速度安全；涉密、根证书等高安全场景选用 4096 位；1024 位已淘汰。
...
> 答：多层嵌套签名（签名的签名）具备实际意义。
适用场景：多级机构授权场景，例如软件分发：开发者 Alice 对程序签名；分发厂商 Bob 对 Alice 的签名二次签名。
作用：多层身份核验，接收方不仅校验开发者原始签名，同时校验分发渠道合法，可实现多级信任链路；常见于证书链、软件渠道分级授权、多级政务文件审批签名场景。
