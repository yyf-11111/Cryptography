...
攻击者截获了如下密文（十六进制编码）：20814804c1767293b99f1d9cab3bc3e7 ac1e37bfb15599e5f40eef805488281d
...
**修改方法：** 通过修改第一个密文块（IV后的第一块）来改变解密后的第一个明文块。CBC模式中，$m_1 = D(k, c_1) \\oplus IV$。要修改 $m_1$ 中的特定字节，只需将IV的对应字节异或上原明文字节和目标明文字节的差值。
...
$$
V(k, m, t) := \\begin{cases} 1 & \\text{若 } D(k, t) = m \\\\ 0 & \\text{否则} \\end{cases}
$$
...
$$
\\Pr[k = c] = \\begin{cases} 1 / 2^{128} & \\text{若 } \\operatorname{MSB}_{128}(c) = 0^{128} \\\\ 0 & \\text{否则} \\end{cases}
$$

设 $F(k, x)$ 是一个输入空间为 $\\{0,1\\}^{256}$ 的安全 PRF。以下哪个构造在密钥 $k$ 从完整密钥空间 $\\{0,1\\}^{256}$ 均匀采样时是安全 PRF，但在密钥从上述非均匀分布采样时是不安全的？

- [ ] $$
  F'(k, x) = \\begin{cases} F(k, x) & \\text{若 } \\operatorname{MSB}_{128}(k) = 0^{128} \\\\ 1^{256} & \\text{否则} \\end{cases}
  $$
...
- [ ] $$
  F'(k, x) = \\begin{cases} F(k, x) & \\text{若 } \\operatorname{MSB}_{128}(k) \\neq 1^{128} \\\\ 0^{256} & \\text{否则} \\end{cases}
  $$
...
- [x] 在同一个密钥下加密数据库中的多条记录，并且相同记录可能重复多次
...
- [x] 不安全，因为对于 $x \\neq x'$，有
  $$
  E'((k_1,k_2),0,x) \\oplus E'((k_1,k_2),1,x) = E'((k_1,k_2),0,x') \\oplus E'((k_1,k_2),1,x')
  $$
...
$$
V(k, m, \\text{tag}) := \\begin{cases} 1 & \\text{若 } E(k, m, 0) = \\text{tag} \\\\ 0 & \\text{否则} \\end{cases}
$$
