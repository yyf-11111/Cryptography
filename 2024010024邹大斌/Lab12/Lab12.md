...
| 项目 | 你的结果 |
| :--- | :------- |
| 证书主题（Subject） |subject=C=CN, ST=Yunnan, L=Yuxi, O=My Company, OU=Web Services, CN=www.myserver.com
 |
| 证书颁发者（Issuer） | C=CN, ST=Yunnan, L=Yuxi, O=My Organization, OU=IT Department, CN=My Intermediate CA
 |
| CA 字段的值 | basicConstraints=CA:FALSE|
...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） | sha256 Fingerprint=14:CE:8A:6B:55:E7:1A:2A:C3:8E:40:D1:59:70:99:4B:47:A9:9F:A5:58:F0:9C:49:4E:10:AD:D7:64:69:9D:7A
|
| SHA-1 指纹（完整） |sha1 Fingerprint=63:A6:D9:17:13:16:5C:FD:C7:79:35:51:66:DD:B5:4C:0D:34:54:2C
 |
...
| :--- | :--- | :--- |
| 最终信任根 |公开可信的第三方根CA | 自己搭建的私有根CA|
| 浏览器默认是否信任 |是 |否 |
| 证书签发者是否为第三方 CA |是 | 否|
| 适合的使用场景 | 公网对外服务|内部测试、局域网服务 |
...
> 答：本质：绑定实体身份与公钥的CA签名电子文件；解决公钥信任问题；数字签名无法证明身份，证书通过CA背书解决身份可信问题。
...
> 答：自签名证书：实体自己签发的证书；浏览器不信任是因为无权威背书；适用内网测试、内部系统
...
> 答：证书链：保护根CA私钥、降低管理成本；根CA直接签发会提升泄露风险和管理成本。
...
> 答：根CA被信任是因为系统/浏览器预装并审核；通过内置根证书列表决定信任。
...
> 答：pathlen:0  表示不能签发下级CA； pathlen:1  可签发1级下级CA，层级限制更宽松。
...
> 答：网站证书过期后，浏览器会拦截访问并提示安全风险，拒绝建立HTTPS加密连接，用户无法正常打开网站。
设置有效期的原因：① 限制密钥泄露后的攻击窗口期，密钥一旦泄露，过期后证书自动失效；② 强制站点定期更新密钥与证书，淘汰老旧不安全加密算法；③ 让CA可以定期核验域名持有者所有权。
Let’s Encrypt设置90天短有效期：倒逼用户自动化续证，降低证书长期被盗用、遗忘更新的风险，同时减轻CA长期吊销维护的压力。
...
> 答：需要吊销证书的场景：服务器私钥泄露、域名所有权变更、企业业务注销、证书申请信息错误。
 
- CRL（证书吊销列表）：CA定期生成全量被吊销证书清单，客户端下载完整列表校验，文件会随吊销数量不断变大，占用带宽与存储，校验慢。
- OCSP（在线证书状态协议）：客户端向CA服务器单独查询单张证书实时状态，按需请求，体积小、效率更高，是现在主流方案。
...
> 答：现代证书普遍使用SAN扩展，是因为早期仅依靠CN（通用名）匹配域名存在兼容性缺陷，浏览器后续标准优先读取SAN字段。
区别：
1. CN：仅支持填写一个主域名，属于旧标准字段，部分新版浏览器不再优先识别；
2. SAN：支持同时绑定多个域名、通配符域名，是现行SSL证书标准扩展，兼容性全覆盖。
...
> 答：证书指纹使用SHA256/SHA1等哈希算法，对整张证书二进制内容做单向哈希计算生成唯一摘要。
证书钉扎（Certificate Pinning）：客户端预先绑定服务器证书公钥指纹，后续连接时强制比对指纹，即便合法CA被劫持签发伪造证书，指纹不匹配就会拒绝连接，抵御中间人伪造证书攻击。
...
> 答：*.example.com  可以匹配  xxx.example.com 、 api.example.com 、 mail.example.com  这类一级子域名；不能匹配根域名 example.com，也不能跨多级子域名（a.b.example.com）。
局限性：只要该通配证书私钥泄露，所有下属一级子域名全部面临劫持风险；无法区分不同子域名独立部署，安全粒度粗。
...
> 答：CT是一套公开日志审计系统，所有公网SSL证书签发后必须提交到公开日志服务器存档。
解决问题：防止CA在不知情的情况下，违规为域名签发伪造证书，域名所有者可以在公开日志里检索自己域名的全部证书，及时发现恶意证书并吊销，提升CA签发行为的可审计性。
...
> 答：Let's Encrypt 依靠ACME协议自动化流程免费签发证书；ACME通过HTTP-01（放置验证文件）或DNS-01（添加TXT记录）验证域名控制权；90天短有效期可降低安全风险、减少维护压力，自动续期确保证书持续有效。
...
> 答：浏览器默认信任 3n7er.space 的Let's Encrypt证书是因为其根证书被浏览器内置信任列表收录，而实验私有证书的根证书未被收录；两者证书格式均遵循X.509标准、都采用层级证书链结构，但信任根、签发者、验证方式和使用场景存在差异。
