...
检查有效期：当前时间是否在 Not Before 和 Not After 之间
检查域名匹配：证书的 CN 或 SAN 字段是否匹配访问的域名（通配符 *.example.com 可匹配二级域名）
验证证书链：用中间 CA 公钥验证网站证书签名 → 用根 CA 公钥验证中间 CA 证书签名 → 根 CA 证书预装在系统中，自动信任
检查吊销状态：通过 CRL（证书吊销列表）或 OCSP（在线证书状态协议）确认证书未被吊销
检查证书用途：Key Usage 和 Extended Key Usage 是否与使用场景匹配
证书指纹（Fingerprint）
...
证书有效期（起始-结束）	Not Before: May 22 12:16:05 2026 GMT  Not After: Aug 20 12:16:04 2026 GMT
公钥算法	id-ecPublicKey
签名算法	ecdsa-with-SHA384
证书链层级数	3 层
根 CA 名称	ISRG Root X1
OpenSSL 验证结果（Verify return code）	Verify return code: 0 (ok)
是否为公开 CA / 第三方 CA 签发	是，由公开免费 CA Let's Encrypt 签发
截图要求
请截图以下内容并保存到同一目录下：
...
密钥长度为什么逐层缩短？ 根 CA 证书有效期 10 年，私钥一旦泄露整条链全部失效，值得用 4096 位；中间 CA 和服务器证书有效期短，到期可以重签，且签名操作更频繁（性能有影响），2048 位是合理的折中。

有效期为什么逐层缩短？ 根 CA 证书一旦过期，所有依赖它的设备都需要更新信任库，成本极高，所以给 10 年。中间 CA 只需要更新 CA 本身的证书，影响面小，5 年。服务器证书最容易出问题（私钥泄露、域名变更），1 年强制更新是对安全的主动保障。
...
keyUsage = critical, digitalSignature, cRLSign, keyCertSign
EOF
pathlen:0：这里改成了 0，比根 CA 的 pathlen:1 少一层。含义是：中间 CA 可以签发证书，但只能签发终端实体证书，不能再签发下一层 CA 证书。这在结构上保证了证书链的深度不会无限延伸
...
根 CA 私钥通常存放在离线的硬件安全模块（HSM）中，不参与日常签名操作，性能不是问题，所以可以用更长的密钥。中间 CA 每天都要签发证书，签名操作频率高，密钥太长会带来明显的性能开销，2048 位在安全与性能之间取得平衡。
...
把服务器证书和中间 CA 证书拼接成一个文件。这是实际部署 HTTPS 服务器时的标准做法：服务器在 TLS 握手时需要把完整的证书链（从自身到中间 CA）发给客户端，客户端才能逐级验证到根 CA。注意顺序是叶子证书在前，中间 CA 在后。
...
这三个值沿着链条一路收紧，体现的是最小权限原则：每一层只被授予完成自身任务所需的最小权限。服务器只需要"证明自己是 www.myserver.com"，不需要签发任何证书，所以 CA:FALSE。
...
E. 证书指纹（任务五）
项目	你的结果
SHA-256 指纹（完整）	35:73:7E:E7:6F:17:99:B1:58:D2:45:B4:F7:89:4E:CA:7C:3C:9D:1F:00:6F:6B:89:CD:7F:F1:B7:F6:B2:54:39
SHA-1 指纹（完整）	71:3C:D7:54:08:4B:45:18:05:70:7B:BE:5E:54:D1:05:86:9F:47:D0
F. 真实 PKI 与私有 PKI 对比
对比项	真实公网 PKI（${SITE}）	本实验私有 PKI
最终信任根		ISRG Root X1	My Root CA
浏览器默认是否信任		是，系统预装根证书，默认信任	否，需手动导入根证书到系统信任库
证书签发者是否为第三方 CA		是，Let's Encrypt 公共第三方 CA	否，自建私有 CA，无第三方机构背书
适合的使用场景		公网对外网站、公开互联网服务	内网业务、实验室测试、企业内部系统、局域网服务
思考题
1. 数字证书的本质
数字证书的本质是什么？它解决了什么问题？为什么单独的数字签名不够，还需要数字证书？

答：
1.本质：数字证书是由可信 CA 签发、绑定实体身份与公钥的标准化电子文件（X.509 格式），包含实体名称、公钥、有效期、CA 签名、扩展约束等信息，是公钥身份的可信凭证。
2.解决的核心问题：解决公钥分发时的中间人攻击—— 单纯传输公钥无法证明公钥归属，攻击者可替换公钥劫持通信；证书通过可信第三方背书，证明公钥确实属于对应域名 / 个人 / 机构。
3.仅数字签名不足的原因：数字签名只能证明 “数据未篡改、签名者持有对应私钥”，但无法验证签名者身份是否可信，无法证明公钥归属；数字证书引入权威 CA 建立信任链，完成身份确权。

2. 自签名证书 vs CA 签名证书
什么是自签名证书？为什么浏览器会警告自签名证书不安全？在什么场景下可以使用自签名证书？

答：
1.自签名证书定义：实体使用自身私钥签发自己的证书，签发者 (issuer) 与主题 (subject) 完全一致，无第三方 CA 背书，信任链只有自身一层。
2.浏览器警告原因：操作系统 / 浏览器预装的可信根库不包含该自签名根证书，无法验证证书合法性，无法抵御中间人伪造证书，判定存在安全风险。
3.适用场景：内网测试环境、实验室教学、局域网内部服务、离线私有系统；公网对外网站严禁使用。

3. 证书链的必要性
为什么需要证书链（根 CA → 中间 CA → 终端实体证书）？为什么不让根 CA 直接签发所有终端实体证书？

答：
1.分层证书链作用（根 CA→中间 CA→终端证书）
安全隔离：根 CA 私钥离线冷存储，极少使用，大幅降低最高信任根泄露风险；中间 CA 在线完成日常证书签发，分担操作风险。
权限分级：通过pathlen控制证书签发层级，限制中间 CA 无限下级签发，防止信任体系失控。
业务分权：不同业务 / 地区使用独立中间 CA，单一中间 CA 泄露不会全局信任崩塌。
2.根 CA 不直接签发终端证书的原因
根私钥一旦泄露，全网所有依赖它的证书全部失效，灾难影响极大；根 CA 仅用来签发少量中间 CA，尽量减少私钥使用次数，提升整体安全。

4. 根 CA 证书的信任
根 CA 证书是自签名的，为什么我们会信任它？操作系统和浏览器如何决定信任哪些根 CA？

答：
1.根 CA 虽是自签名，但由操作系统、浏览器厂商人工审核、预装到本地可信根证书库，厂商背书其身份合法性，因此系统默认信任。
2.信任判定机制：
系统内置可信根列表，启动 / 浏览器初始化时加载；
验证证书链时，若整条链路最终能追溯到列表内的根证书，则判定合法可信；
不在内置列表的自签名根，一律标记不可信，弹出安全告警。

5. pathlen 的作用
中间 CA 证书中的 pathlen:0 是什么意思？如果 pathlen:1，会有什么区别？

答：pathlen 用于约束该 CA 证书最多能向下签发几层子 CA，管控证书链深度：
1.pathlen:0：当前 CA不能再签发任何下级中间 CA，仅能直接签发服务器 / 用户终端证书（本次实验中间 CA 配置为此值）。
2.pathlen:1：当前 CA 最多可以签发 1 层中间 CA，该中间 CA 下不能再创建子 CA，整体链路最大深度为 根 CA→本 CA→下级中间 CA→终端证书。
3.核心意义：防止无限分层创建 CA，缩小私钥泄露后的信任破坏范围。

6. 证书过期
如果一个网站的证书过期了，会发生什么？为什么证书需要有效期限制？为什么 Let's Encrypt 的证书只有 90 天有效期？

答：
1.过期后果：浏览器 / 客户端直接拦截访问，弹出安全错误，TLS 握手失败，网站无法正常 HTTPS 访问。
2.设置有效期的原因：
加密密钥存在泄露风险，定期更换密钥降低长期泄露危害；
实体信息（域名、企业信息）会变更，过期强制更新证书同步最新信息；
缩小证书泄露后的有效攻击窗口期。
3.Let's Encrypt 仅 90 天有效期：
依托 ACME 自动化续期工具，短周期强制自动更新，即便证书 / 密钥泄露，有效攻击窗口仅 90 天；同时减轻 CRL 吊销列表存储压力，降低运维成本。

7. 证书吊销
什么情况下需要吊销证书？CRL 和 OCSP 有什么区别？

答：
1.需要吊销的场景：私钥泄露、域名所有权变更、企业注销、证书申请信息造假、证书提前作废。
2.CRL 与 OCSP 区别：
CRL（证书吊销列表）CA 定期发布完整吊销证书清单，客户端完整下载列表本地比对下载文件体积大、更新延迟，离线可用。
OCSP（在线证书状态协议）客户端单独向 OCSP 服务器查询单张证书状态单次查询流量小、实时状态，必须联网。

8. Subject Alternative Name
为什么现代证书通常包含 SAN 扩展？SAN 和 CN 有什么区别？

答：
1.现代证书必须包含 SAN 扩展：早期仅依靠 CN 字段匹配域名，标准更新后浏览器不再信任仅靠 CN 的证书，强制读取 SAN 字段校验域名。
2.SAN 与 CN 区别：
CN：仅支持填写 1 个主域名，存在兼容性缺陷，新标准不再作为域名校验依据；
SAN：可同时填写多个独立域名、通配符域名，是当前域名匹配的标准字段，一张证书可服务多个站点。

9. 证书指纹的用途
证书指纹是如何计算的？什么是证书钉扎（Certificate Pinning）？

答：
1.计算方式：对完整 DER 编码的证书二进制数据，做 SHA1/SHA256 哈希运算，输出十六进制冒号分隔字符串，唯一标识一张证书。
2.证书钉扎（Certificate Pinning）：客户端 / 服务端预先绑定服务器证书指纹，仅信任匹配该指纹的证书；即使公根 CA 被攻破、伪造证书，只要指纹不匹配就拒绝连接，抵御中间人攻击。常用于 APP、内网高安全服务。
...
答：
1.*.example.com 匹配规则：
可匹配 www.example.com、mail.example.com、test.example.com；不匹配根域名 example.com，也不匹配多级子域名 a.www.example.com。
2.局限性：
仅覆盖一级子域名，无法匹配二级及更深子域名；
私钥泄露后，该域名下所有子网站全部存在风险；
无法区分不同子站业务，单一证书泄露影响全部子服务。
...
答：
1.CT 定义：一套公开、可审计的证书日志系统，所有公网可信 CA 签发的证书必须提交至公共 CT 日志服务器，生成 SCT（签名时间戳）嵌入证书。
2.解决问题：
防止 CA 违规签发虚假域名证书（恶意中间人证书），任何人可检索日志发现异常证书；
浏览器校验证书内携带的有效 SCT，未上传 CT 日志的证书会被拦截；
域名所有者可监控日志，及时发现冒用自身域名的非法证书。
...
答：
1.免费签发逻辑（对应 3n7er.space 证书链：ISRG Root X1 → E7 中间 CA → 站点证书）：
Let's Encrypt 由非营利组织 ISRG 运营，根证书 ISRG Root X1 预装在全球系统信任库；通过自动化 ACME 协议代替人工审核，无人工运营成本，因此免费签发证书。
2.ACME 域名所有权验证两种主流方式：
HTTP-01：在网站根目录放置指定随机验证文件，ACME 服务器访问http://域名/.well-known/读取文件，证明你拥有网站服务器控制权；
DNS-01：在域名 DNS 添加指定 TXT 记录，校验记录内容，适合无公网 80 端口、通配符域名场景。
3.短有效期 + 自动续期原因：
90 天缩短密钥泄露攻击窗口；依托 Certbot 等工具全自动续期，无需人工操作，规避用户忘记更新证书导致网站瘫痪。
...
答：
一、信任差异原因
浏览器默认信任 3n7er.space 证书：其证书链根为 ISRG Root X1，预装在系统可信根库；
不信任实验私有证书：自建 My Root CA 未加入系统预装信任列表，无厂商官方背书。
二、相同点
1.证书格式统一：均遵循 X.509 v3 标准，包含 subject、issuer、有效期、公钥、v3 扩展、CA 签名；
2.证书链结构一致：都采用「根 CA→中间 CA→终端服务器证书」三级分层架构；
3.密码算法体系一致：支持 RSA/ECC 公钥、SHA256 签名、相同的 Basic Constraints/SAN 扩展规则。
三、不同点
1.信任根：真实 PKI 根为全球预装公共根；私有 PKI 根为本地自建，需手动导入信任；
2.签发主体：真实由合规第三方公共 CA；私有由个人 / 企业自建 CA，无第三方审计；
3.使用范围：真实 PKI 适用于公网对外站点；私有 PKI 仅适用于内网、实验室、离线系统；
4.浏览器默认策略：真实证书直接信任；私有证书默认告警拦截。
